...
@Autonomous(name="TestTraiectorie High Departe Dreapta", group="Linear Opmode")
@Config
public class TestTraiectorieHighDeparteDreapta extends LinearOpMode {
    ...
    public static double highDeparteX = 53;
    public static double highDeparteY = 24;
    public static double highDeparteRot= 130;
    public static double highDeparteTg = 130;
    ...
    public static double  UNGHI_TG_1 = -130;
    ...
    public void main(){
        ...
        drive.update();
        Trajectory second = drive.trajectoryBuilder(drive.getPoseEstimate(),Math.toRadians(UNGHI_TG_1))
        ...
        for(int i=0;i<5;i++) {
            third = drive.trajectoryBuilder(new Pose2d(cycle_x, cycle_y, Math.toRadians(cycle_rot)), false)
                    .lineToLinearHeading(new Pose2d(highDeparteX+2*i, highDeparteY, Math.toRadians(highDeparteRot)))
                    .build();
            drive.followTrajectory(third);
            second = drive.trajectoryBuilder(drive.getPoseEstimate(), Math.toRadians(UNGHI_TG_1))
                    .splineToSplineHeading(new Pose2d(precycleX+2*i, precycleY, Math.toRadians(precycleRot)), Math.toRadians(UNGHI_TG_2))
                    //.lineToLinearHeading(new Pose2d(34+3,-63+50,Math.toRadians(180)))
                    .splineTo(new Vector2d(cycle_x + 2*i, cycle_y), Math.toRadians(-90))
                    //.lineToLinearHeading(new Pose2d(precycleX,precycleY,Math.toRadians(precycleRot)))
                    //.splineToLinearHeading()
                    .build();
            drive.followTrajectory(second);
