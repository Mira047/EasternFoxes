...
@Autonomous(name="Auto2023 Stanga High", group="Linear Opmode")
@Config
public class Auto2023Stanga extends LinearOpMode {
    ...
    public static double clawRightClose = 0.32;
    ...
    void parcare(int cycle){
        ...
        if(caz == 3){
            Trajectory parcare = drive.trajectoryBuilder(drive.getPoseEstimate())
                    ...
                    .lineToLinearHeading(new Pose2d(48.5,-25.5,Math.toRadians(0)))
                    .build();
            drive.followTrajectory(parcare);
            drive.update();
            Pose2d cPos = drive.getPoseEstimate();
            if(distance2D(cPos,new Pose2d(48.5,-25.5)) > 30){
            ...
            Trajectory parcare = drive.trajectoryBuilder(drive.getPoseEstimate(),Math.toRadians(120))
                    .splineToSplineHeading(new Pose2d(precycleX - cycle * 0.5,precycleY,Math.toRadians(precycleRot)),Math.toRadians(0))
                    //.lineToLinearHeading(new Pose2d(34+3,-63+50,Math.toRadians(180)))
                    .splineTo(new Vector2d(cycle_x - cycle * 0.5,cycle_y),Math.toRadians(0))
        ...
        for(int i=0;i<CYCLES && opModeIsActive() && !isStopRequested();i++){
            ...
            if(i < 4) {
                traj = drive.trajectoryBuilder(drive.getPoseEstimate(), Math.toRadians(120))
                        .splineToSplineHeading(new Pose2d(precycleX - i * 0.55, precycleY, Math.toRadians(precycleRot)), Math.toRadians(90))
                        //.lineToLinearHeading(new Pose2d(34+3,-63+50,Math.toRadians(180)))
                        .splineTo(new Vector2d(cycle_x - i * 0.55, cycle_y), Math.toRadians(90))
                        //.lineToLinearHeading(new Pose2d(precycleX,precycleY,Math.toRadians(precycleRot)))
                        //.splineToLinearHeading()
                        .build();
            } else {
                traj = drive.trajectoryBuilder(drive.getPoseEstimate(), Math.toRadians(120))
                        .splineToSplineHeading(new Pose2d(precycleX - i * 0.55, precycleY, Math.toRadians(precycleRot)), Math.toRadians(90))
                        //.lineToLinearHeading(new Pose2d(34+3,-63+50,Math.toRadians(180)))
                        .splineTo(new Vector2d(cycle_x - i * 0.55, cycle_y - 1), Math.toRadians(90))
                        //.lineToLinearHeading(new Pose2d(precycleX,precycleY,Math.toRadians(precycleRot)))
                        //.splineToLinearHeading()
                        .build();
            }
            Pose2d targetCyclePos = new Pose2d(cycle_x - i * 0.55,cycle_y,Math.toRadians(-90));
            drive.followTrajectoryAsync(traj);
            traj = drive.trajectoryBuilder(targetCyclePos,false)
                    .lineTo(new Vector2d(precycleX - i * 0.55 ,precycleY))
                    .splineToSplineHeading(new Pose2d(preloadX - i * 0.55,preloadY,Math.toRadians(preloadRot)),Math.toRadians(preloadTg))
                    .build();
            while (drive.isBusy()) {
                Wait(1);
                drive.update();
