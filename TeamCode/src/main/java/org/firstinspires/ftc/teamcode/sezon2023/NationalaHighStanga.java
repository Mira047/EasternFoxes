...
@Autonomous(name="NationalaStangaHigh", group="Linear Opmode")
@Config
public class NationalaHighStanga extends LinearOpMode {
    ...
    void parcare(int cycle){
        ...
        if(caz == 3){
            ...
            Trajectory parcare = drive.trajectoryBuilder(drive.getPoseEstimate())
                    ...
                    .build();
            drive.followTrajectory(parcare);
            while(isStopRequested() == false && opModeIsActive() && brat.getCurrentPosition() > 100 && lift.getCurrentPosition() > 50){
                Wait(1);
            }
            Wait(50);
        }else  if(caz == 1){
            ...
            Trajectory parcare = drive.trajectoryBuilder(drive.getPoseEstimate(),Math.toRadians(120))
                    ...
                    .splineTo(new Vector2d(cycle_x - cycle * 0.5,cycle_y),Math.toRadians(90))
    ...
    public static double cycleY = 1.3;
    public static double cycleRot = -37;
    public static double cycleTg = -37;
    ...
    public static double cycle_x_init=50;
    public static double cycle_y_init = 24.7;
    ...
    void puneCon(int cycle){
        ...
        while (!(p >= t - tresh && p <= t + tresh)) {
            ...
            if(cTime-sTime > 100){
                ...
                if(Math.abs(p-plast) < 20){
                    ...
                    brat.setTargetPosition(bratUp-200);
                    drive.setWeightedDrivePower(new Pose2d(0,0,0.6));
                    Wait(150);
        ...
        drive.setWeightedDrivePower(new Pose2d(-1, 0, -0.5));
        drive.update();
        Wait(50);
        //clawLeft.setPosition(clawLeftClose);
        //clawRight.setPosition(clawRightClose);
        Wait(autoPutDelay-50);
        drive.setWeightedDrivePower(new Pose2d(0, 0, 0));
        drive.update();
    ...
    void setMecanismeDupaCon(int cycle){
        brat.setTargetPosition(0);
        lift.setTargetPosition(LIFT_CYCLES[cycle]);
        clawLeft.setPosition(clawLeftClose-0.1);
        clawRight.setPosition(clawRightClose+0.1);
            clawRotate.setPosition(0.521);
        ghidaj.setPosition(ghidajIdle);
    }
    ...
    private  void main(){
        ...
        for(int currentCycle = 0;currentCycle<5;currentCycle++){
            ...
            Trajectory luatCon;
            if(currentCycle == 0) {
                luatCon = drive.trajectoryBuilder(new Pose2d(preloadX,preloadY, Math.toRadians(preloadRot)), Math.toRadians(120))
